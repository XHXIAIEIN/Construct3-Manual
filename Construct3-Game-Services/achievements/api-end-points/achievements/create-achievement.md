...
This end point allows you to create a new [achievement](../../../achievements/api-objects/achievement.md).
...
```none
https://achievements.construct.net/createachievement.json
```
...
The name of the achievement you want to create.  Must be at least `1` character long and no more than `128` characters long.  Names do not need to be unique.

**description** `string`  
A description of the achievement.  Cannot exceed more than `512` characters long.

**language** `string`  
The language ISO this achievements name and description are written in.  Must be one of `AR`, `BG`, `CS`, `DA`, `DE`, `EL`, `EN`, `ES`, `ET`, `FI`, `FR`, `HE`, `HU`, `ID`, `IT`, `JA`, `KO`, `LT`, `LV`, `NB`, `NL`, `PL`, `PT`, `RO`, `RU`, `SK`, `SL`, `SV`, `TH`, `TR`, `UK`, `VI`, `ZH`.  Defaults to your games language if not specified.

**clientProgressAllowed** [`bool`](../../../data-types.md#internalH1Link4)  
If true, client authenticated requests can award and progress achievements.  If false, all awards + progress can only be made by requests using secret API keys.
...
If true, all requests to get or list this achievement will not return this achievement unless the request is authenticated with a secret API key.

**maxUnlocks** `int32`  
The maximum number of times this achievement can be unlocked.  Defaults to `1` if not specified.

**progressionRequired** `int64`  
If this is a progressive achievement, the amount of progression required to unlock the achievement.

**xpBonuses** `string`  
The XP bonus when the achievement is unlocked.  Send a single `int` value to award the same XP every time this achievement is unlocked.  If this achievement can be unlocked multiple times, you can send comma separated values such as `1=100,11=200,101=1000`.  This awards `100` XP for the first 10 of this achievement awarded, `200` XP for the next 90 and so on.
...
```json
{
  ...
  "helpURL": "https://www.construct.net/en/game-services/manuals/game-services/achievements/api-end-points/achievements/create-achievement",
  "shouldRetry": false
}
```
